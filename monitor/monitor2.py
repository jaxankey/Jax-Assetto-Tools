...
class Monitor:
    ...
    def premium_get_latest_data(self):
        ...
        # Track what changed
        laps_or_onlines_changed = False
        event_time_slots_changed = False
        track_changed = False
        ...
        # Process race.json
        try:
            race_json = c = load_json(CONFIG['path_race_json'])
            
            if CONFIG['path_race_json'] and not c:
                log('WARNING: path_race_json bad\n  ', CONFIG['path_race_json'])
            ...
            if c is not None and 'SignUpForm' in c:
                ...
                CONFIG['qual_minutes'] = c['Events'][0]['RaceSetup']['Sessions']['QUALIFY']['Time']
                ...
                if (tq != self['qual_timestamp'] or 
                    tr != self['race_timestamp'] or 
                    ns != self['number_slots']):
                    event_time_slots_changed = True
        ...
        if (track_changed or carset_fully_changed) and \
        ...
        if self.first_run or \
           laps_or_onlines_changed or \
           track_changed or \
           carset_fully_changed or \
           event_time_slots_changed or \
           session_changed or \
