...
class Modder:
    ...
    def __init__(self, blocking=False, reset=True):

        if reset: 
            print('RESET: Clearing GUI settings...')
            egg.gui.clear_egg_settings()
        ...
        # This will hold all the data from the above ini files.
    ...
    def update_cars(self):
        ...
        # Get the current car name so we can remember what we had after we rebuild the combo list
        og_name = self.combo_car.get_text()

        # Make sure we have a valid assetto path
        while not os.path.exists(os.path.join(self.text_local(), 'content', 'cars')):
            path = egg.dialogs.select_directory(text='Select the Assetto Corsa directory, apex-nerd.', default_directory='assetto_local')
            if path: self.text_local(path)
            else:  
                self.log('No valid path selected.')
                return
